...
<section class="couponzone_list">
    ...
    for($i=0; $row=sql_fetch_array($result); $i++) {
        ...
        switch($row['cp_method']) {
            case '0':
                $sql3 = " select it_id, it_name from {$g5['g5_shop_item_table']} where it_id = '{$row['cp_target']}' and site_id = '$site_id' ";
                $row3 = sql_fetch($sql3);
                $cp_target = '<a href="./item.php?it_id='.$row3['it_id'].'">'.get_text($row3['it_name']).'</a>';
                break;
            case '1':
                $sql3 = " select ca_id, ca_name from {$g5['g5_shop_category_table']} where ca_id = '{$row['cp_target']}' and site_id = '$site_id' ";
                $row3 = sql_fetch($sql3);
