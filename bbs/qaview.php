...
if(is_file($skin_file)) {
    $sql = " select * from {$g5['qa_content_table']} where qa_id = '$qa_id' and site_id = '$site_id' ";
    if(!$is_admin) {
    ...
    $sql = " select qa_id, qa_subject
                ...
                where qa_type = '0' and site_id = '$site_id' ";
    if(!$is_admin) {
        $sql .= " and mb_id = '{$member['mb_id']}' ";
    }
    ...
    */
    ...
    if(!$view['qa_type'] && $view['qa_status']) {
        $sql = " select *
                    from {$g5['qa_content_table']}
                    where qa_type = '1'
                      and qa_parent = '{$view['qa_id']}' and site_id = '$site_id' ";
