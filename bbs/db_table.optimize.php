...
if($config['cf_visit_del'] > 0) {
    ...
    $sql = " delete from {$g5['visit_table']} where vi_date < '$tmp_before_date' and site_id = '$site_id' ";
    sql_query($sql);
...
if($config['cf_popular_del'] > 0) {
    ...
    $sql = " delete from {$g5['popular_table']} where pp_date < '$tmp_before_date' and site_id = '$site_id' ";
    sql_query($sql);
...
if($config['cf_new_del'] > 0) {
    $sql = " delete from {$g5['board_new_table']} where (TO_DAYS('".G5_TIME_YMDHIS."') - TO_DAYS(bn_datetime)) > '{$config['cf_new_del']}' and site_id = '$site_id' ";
    sql_query($sql);
...
if($config['cf_memo_del'] > 0) {
    $sql = " delete from {$g5['memo_table']} where (TO_DAYS('".G5_TIME_YMDHIS."') - TO_DAYS(me_send_datetime)) > '{$config['cf_memo_del']}' and site_id = '$site_id' ";
    sql_query($sql);
...
if($config['cf_leave_day'] > 0) {
    $sql = " select mb_id from {$g5['member_table']}
                where (TO_DAYS('".G5_TIME_YMDHIS."') - TO_DAYS(mb_leave_date)) > '{$config['cf_leave_day']}'
                  and mb_memo not regexp '^[0-9]{8}.*삭제함' and site_id = '$site_id' ";
...
if(isset($config['cf_optimize_date'])) {
    sql_query(" update {$g5['config_table']} set cf_optimize_date = '".G5_TIME_YMD."' and site_id = '$site_id' ");
}
?>