...
class item_list
{
    ...
    function run() {
        ...
        global $g5, $config, $member, $default, $site_id;
        ...
        if ($this->query) {
            ...
            if ($this->event) {
                ...
                $where[] = " a.ev_id = '{$this->event}' and a.site_id = '$site_id' and b.site_id = '$site_id' ";
            } else {
                $sql_select = " select {$this->fields} ";
                $sql_common = " from `{$g5['g5_shop_item_table']}` ";
            }
            $sql_where = " where " . implode(" and ", $where);
            if (!$this->event) { // 이벤트가 아니라면 join을 하지않으므로 where절에 site_id 추가(gnuwiz)
...
function get_cart_count($cart_id)
{
    ...
    $sql = " select count(ct_id) as cnt from {$g5['g5_shop_cart_table']} where od_id = '$cart_id' and site_id = '$site_id' ";
    $row = sql_fetch($sql);
    $cnt = (int)$row['cnt'];
    return $cnt;
}
...
function get_it_image($it_id, $width, $height=0, $anchor=false, $img_id='', $img_alt='', $is_crop=false)
{
    ...
    $sql = " select it_id, it_img1, it_img2, it_img3, it_img4, it_img5, it_img6, it_img7, it_img8, it_img9, it_img10 from {$g5['g5_shop_item_table']} where it_id = '$it_id' and site_id = '$site_id' ";
    $row = sql_fetch($sql);
...
function print_item_options($it_id, $cart_id)
{
    ...
    $sql = " select ct_option, ct_qty, io_price
                from {$g5['g5_shop_cart_table']} where it_id = '$it_id' and od_id = '$cart_id' and site_id = '$site_id' order by io_type asc, ct_id asc ";
    $result = sql_query($sql);
...
function display_banner($position, $skin='')
{
    ...
    if(file_exists($skin_path)) {
        ...
        // 배너 출력
        $sql = " select * from {$g5['g5_shop_banner_table']} where '".G5_TIME_YMDHIS."' between bn_begin_time and bn_end_time $sql_device and bn_position = '$position' and site_id = '$site_id' order by bn_order, bn_id desc ";
        $result = sql_query($sql);
...
function is_used_coupon($mb_id, $cp_id)
{
    ...
    $sql = " select count(*) as cnt from {$g5['g5_shop_coupon_log_table']} where mb_id = '$mb_id' and cp_id = '$cp_id' and site_id = '$site_id' ";
    $row = sql_fetch($sql);
...
function is_coupon_downloaded($mb_id, $cz_id)
{
    ...
    $sql = " select count(*) as cnt from {$g5['g5_shop_coupon_table']} where mb_id = '$mb_id' and cz_id = '$cz_id' and site_id = '$site_id' ";
    $row = sql_fetch($sql);
