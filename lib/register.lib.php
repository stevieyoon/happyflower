...
function exist_mb_id($reg_mb_id)
{
    ...
    $sql = " select count(*) as cnt from `{$g5['member_table']}` where mb_id = '$reg_mb_id' and site_id = '$site_id' ";
    $row = sql_fetch($sql);
    if ($row['cnt'])
...
{
    global $g5, $site_id;
    $row = sql_fetch(" select count(*) as cnt from {$g5['member_table']} where mb_nick = '$reg_mb_nick' and mb_id <> '$reg_mb_id' and site_id = '$site_id' ");
    if ($row['cnt'])
...
{
    global $g5, $site_id;
    $row = sql_fetch(" select count(*) as cnt from `{$g5['member_table']}` where mb_email = '$reg_mb_email' and mb_id <> '$reg_mb_id' and site_id = '$site_id' ");
    if ($row['cnt'])
...
function exist_mb_hp($reg_mb_hp, $reg_mb_id)
{
    ...
    $sql = "select count(*) as cnt from {$g5['member_table']} where mb_hp = '$reg_mb_hp' and mb_id <> '$reg_mb_id' and site_id = '$site_id' ";
    $row = sql_fetch($sql);
