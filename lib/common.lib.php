...
function insert_point($mb_id, $point, $content='', $rel_table='', $rel_id='', $rel_action='', $expire=0)
{
    ...
    if ($mb_id == '') { return 0; }
    $mb = sql_fetch(" select mb_id from {$g5['member_table']} where mb_id = '$mb_id' and site_id = '{$site_id}' ");
    ...
    // 포인트 UPDATE
    $sql = " update {$g5['member_table']} set mb_point = '$po_mb_point' where mb_id = '$mb_id' and site_id = '{$site_id}' ";
    sql_query($sql);
...
function get_uniqid()
{
    ...
    while (1) {
        ...
        $result = sql_query(" insert into {$g5['uniqid_table']} set uq_id = '$key', uq_ip = '{$_SERVER['REMOTE_ADDR']}', site_id = '$site_id' ", false);
...
class html_process {
    ...
    function run()
    {
        global $config, $g5, $member, $site_id;

        // 현재접속자 처리
        $tmp_sql = " select count(*) as cnt from {$g5['login_table']} where lo_ip = '{$_SERVER['REMOTE_ADDR']}' and site_id = '$site_id' ";
        $tmp_row = sql_fetch($tmp_sql);

        if ($tmp_row['cnt']) {
            $tmp_sql = " update {$g5['login_table']} set mb_id = '{$member['mb_id']}', lo_datetime = '".G5_TIME_YMDHIS."', lo_location = '{$g5['lo_location']}', lo_url = '{$g5['lo_url']}' where lo_ip = '{$_SERVER['REMOTE_ADDR']}' and site_id = '$site_id' ";
            sql_query($tmp_sql, FALSE);
        } else {
            $tmp_sql = " insert into {$g5['login_table']} ( lo_ip, mb_id, lo_datetime, lo_location, lo_url, site_id ) values ( '{$_SERVER['REMOTE_ADDR']}', '{$member['mb_id']}', '".G5_TIME_YMDHIS."', '{$g5['lo_location']}',  '{$g5['lo_url']}', '$site_id' ) ";
            sql_query($tmp_sql, FALSE);

            // 시간이 지난 접속은 삭제한다
            sql_query(" delete from {$g5['login_table']} where lo_datetime < '".date("Y-m-d H:i:s", G5_SERVER_TIME - (60 * $config['cf_login_minutes']))."' and site_id = '$site_id' ");
...
function get_qa_config($fld='*')
{
    ...
    $sql = " select $fld from {$g5['qa_config_table']} where site_id = '$site_id' ";
    $row = sql_fetch($sql);
...
function insert_popular($field, $str)
{
    ...
    if(!in_array('mb_id', $field)) {
        $sql = " insert into {$g5['popular_table']} set pp_word = '{$str}', pp_date = '".G5_TIME_YMD."', pp_ip = '{$_SERVER['REMOTE_ADDR']}', site_id = '$site_id' ";
        sql_query($sql, FALSE);
    }
}
...
//////////////////////////////////////////////////////////////
// extend/user.config.php 파일에 넣으려 했으나 상단에 있어야 하기 때문에 부득이하게 common.lib.php 파일이 추가
...
// 플랫폼 사업자의 고유 번호를 가져옴
function get_auth(){
	global $g5, $config, $current_server_name;
