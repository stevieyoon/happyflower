...
else if ($w == "u")
{
    ...
    $sql = " update {$g5['g5_shop_item_use_table']}
                set is_subject = '$is_subject',
                    is_content = '$is_content',
                    is_score = '$is_score'
              where is_id = '$is_id' and site_id = '$site_id' ";
    sql_query($sql);
...
else if ($w == "d")
{
    if (!$is_admin)
    {
        $sql = " select count(*) as cnt from {$g5['g5_shop_item_use_table']} where mb_id = '{$member['mb_id']}' and is_id = '$is_id' and site_id = '$site_id' ";
        $row = sql_fetch($sql);
        if (!$row['cnt'])
    ...
    // 에디터로 첨부된 이미지 삭제
    $sql = " select is_content from {$g5['g5_shop_item_use_table']} where is_id = '$is_id' and md5(concat(is_id,is_time,is_ip)) = '{$hash}' and site_id = '$site_id' ";
    $row = sql_fetch($sql);
    ...
    $sql = " delete from {$g5['g5_shop_item_use_table']} where is_id = '$is_id' and md5(concat(is_id,is_time,is_ip)) = '{$hash}' and site_id = '$site_id' ";
    sql_query($sql);
