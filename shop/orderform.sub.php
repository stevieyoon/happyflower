    ...
    <div class="tbl_head03 tbl_wrap od_prd_list">
        ...
        $sql = " select a.ct_id,
                  ...
                  where a.od_id = '$s_cart_id'
                    and a.ct_select = '1' and a.site_id = '$site_id' ";
        ...
        for ($i=0; $row=sql_fetch_array($result); $i++)
        {
            ...
            $sql = " select SUM(IF(io_type = 1, (io_price * ct_qty), ((ct_price + io_price) * ct_qty))) as price,
                        ...
                        where it_id = '{$row['it_id']}'
                          and od_id = '$s_cart_id' and site_id = '$site_id' ";
            $sum = sql_fetch($sql);
        ...
        <section id="sod_frm_taker">
            ...
            <div class="tbl_frm01 tbl_wrap">
                ...
                <tr>
                    ...
                    <td>
                        <input type="text" name="od_hope_date" value="" id="od_hope_date" required class="frm_input required" size="11" maxlength="10" readonly="readonly">
						<!-- 배송 시간 추가 시작(gnuwiz) -->
						<select name="od_hope_time" id="od_hope_time" class="frm_input required" >
						...
						<input type="checkbox" name="od_hope_now" id="od_hope_now" value="1">
						<label for="od_hope_now" style="color:red;">즉시배송</label>
						<script>
						function mag_check(mode) {
							if(mode == "ribon") {
								$(".card_message").css("display","none");
								$(".ribbon_message").css("display","block");
							}else if(mode == "card_ribon") {
								$(".card_message").css("display","block");
								$(".ribbon_message").css("display","block");
							}else {
								$(".card_message").css("display","block");
								$(".ribbon_message").css("display","none");
							}
						}
						...
						$(function () {
							...
							var  st = this.checked;
								if (st) {
                                    ...
                                    $("[name='od_hope_date']").removeAttr("required").removeClass("required");
                                    $("[name='od_hope_time']").removeAttr("required").removeClass("required");
								  } else {
                                    ...
                                    $("[name='od_hope_date']").attr("required", true).addClass("required");
                                    $("[name='od_hope_time']").attr("required", true).addClass("required");
								}
							});
						});
						</script>
				...
				<?php
				$sql = " select * from {$g5['g5_shop_cart_table']} where od_id = '$s_cart_id' and io_type = '0' and site_id = '$site_id' order by ct_id";
				$resa = sql_query($sql);
				for($m=0; $ga=sql_fetch_array($resa); $m++) {
				$ct_qty = (int)$ga['ct_qty'];
				  if($ct_qty) {
					 for($k=0;$k<$ct_qty;$k++){
						$v += 1;
						$p += 20;
						...
						// 작은 이미지
						$image = get_it_image($ga['it_id'], 70, 80);
				?>
				<tr class="">
					<th>리본 문구 <?php echo $v; ?></th>
					<td colspan="3">
					<table>
					<tbody>
					<tr>
						<td rowspan="2">
							<a href="<?php echo G5_SHOP_URL ?>/largeimage.php?it_id=<?php echo $ga['it_id'] ?>&amp;no=1" target="_blank" class="popup_item_image img_thumb"><?php echo $image; ?></a>
						</td>
						<td>
                            <input type="radio" id="ribbon_type_1[<?php echo $v; ?>]" name="ribbon_type[<?php echo $v; ?>]" class="" onclick="card_ribbon<?php echo $v; ?>('<?php echo $v; ?>')" value="0" checked> <label for="ribbon_type_1[<?php echo $v; ?>]">리본 메세지</label>
                            <input type="radio" id="ribbon_type_2[<?php echo $v; ?>]" name="ribbon_type[<?php echo $v; ?>]" onclick="card_ribbon<?php echo $v; ?>('<?php echo $p; ?>');" value="1"> <label for="ribbon_type_2[<?php echo $v; ?>]">리본 / 축하카드 메세지</label>
						</td>
					</tr>
					<tr>
						<td>
                            <div class="<?php echo $ga['it_id']; ?>_<?php echo $v; ?>" />
                                <input type="hidden" name="ribbon[<?php echo $v; ?>]" />
                                <input type="hidden" name="it_id[<?php echo $v; ?>]" value="<?php echo $ga['it_id']; ?>" />
                                <input type="hidden" name="it_name[<?php echo $v; ?>]" value="<?php echo $ga['it_name']; ?>" />
                                <input name="rb_left_content[<?php echo $v; ?>]" class="frm_input ribbonmsg ribbon<?php echo $v; ?>"  placeholder="경조사어->왼쪽리본메세지(<?php echo $ga['it_name']; ?>)"  />
                                <button type="button" class="btn_address" onclick="window.open('<?=G5_SHOP_URL?>/ribbon_content.php?no=<?php echo $v; ?>&class_name=ribbon<?php echo $v; ?>','ribbon<?php echo $v; ?>','width=600,height=500,scrollbars=yes,resizable=no,status=yes');">문구보기</button>
                                <br />
                                <input name="rb_right_content[<?php echo $v; ?>]"  class="frm_input ribbonmsg1" placeholder="보내는이->오른쪽리본메세지"  />
							</div>
							<div class="dinone <?php echo $ga['it_id']; ?>_<?php echo $p; ?>">
                                <textarea name="card_msg[<?php echo $v; ?>]" class="frm_input cardmsg cardmsg<?php echo $v; ?>" placeholder="카드메세지"></textarea>
                                <button type="button" class="btn_address" onclick="window.open('<?=G5_SHOP_URL?>/card_content.php?no=<?php echo $v; ?>&class_name=cardmsg<?php echo $v; ?>','','width=600,height=500,scrollbars=yes,resizable=yes,status=yes');">문구보기</button>
							</div>
						</td>
					</tr>
				...
				<script>
				function card_ribbon<?php echo $v; ?>(no){
					if(no=="<?php echo $v; ?>"){
						$(".<?php echo $ga['it_id']; ?>_<?php echo $v; ?>").show();
						$(".<?php echo $ga['it_id']; ?>_<?php echo $p; ?>").hide();
					}else if(no=="<?php echo $p; ?>"){
						$(".<?php echo $ga['it_id']; ?>_<?php echo $v; ?>").show();
						$(".<?php echo $ga['it_id']; ?>_<?php echo $p; ?>").show();
					}
				}
				</script>
				<?php
				...
				<tr>
                    ...
                    <td colspan="3">
						<input type="radio" id="od_receipt_type_1" name="od_receipt_type" value="세금계산서"> <label for="od_receipt_type_1">세금계산서</label>
						<input type="radio" id="od_receipt_type_2" name="od_receipt_type" value="현금영수증(사업자)"> <label for="od_receipt_type_2">현금영수증(사업자)</label>
						<input type="radio" id="od_receipt_type_3" name="od_receipt_type" value="현금영수증(전화번호)"> <label for="od_receipt_type_3">현금영수증(전화번호)</label>
						<input type="radio" id="od_receipt_type_4" name="od_receipt_type" value="간이영수증"> <label for="od_receipt_type_4">간이영수증</label>
						<input type="radio" id="od_receipt_type_5" name="od_receipt_type" value="카드영수증"> <label for="od_receipt_type_5">카드영수증</label>
					</td>
                </tr>
				<script>
				...
				<tr id="receipt_type1" style="display:none;">
					...
					<td colspan="3">
						<table class="cebox1">
							<tr>
								<th>사업자 번호</th>
								<td colspan="3"><input type="text" id="od_receipt_saupja_no" name="od_receipt_saupja_no" value="" class="frm_input"></td>
							</tr>
							<tr>
								<th>회사명</th>
								<td><input type="text" id="od_receipt_company_name" name="od_receipt_company_name" value="" class="frm_input"></td>
								<th>대표자명</th>
								<td><input type="text" id="od_receipt_company_owner" name="od_receipt_company_owner" value="" class="frm_input"></td>
							</tr>
							<tr>
								<th>주소</th>
								<td colspan="3"><input type="text" id="od_receipt_company_addr" name="od_receipt_company_addr" value="" class="frm_input" size="45" /></td>
							</tr>
							<tr>
								<th>업태</th>
								<td><input type="text" id="od_receipt_company_uptae" name="od_receipt_company_uptae" value="" class="frm_input" /></td>
								<th>종목</th>
								<td><input type="text" id="od_receipt_company_upjong" name="od_receipt_company_upjong" value="" class="frm_input" /></td>
							</tr>
							<tr>
								<th>이메일</th>
								<td colspan="3"><input type="text" id="od_receipt_company_email" name="od_receipt_company_email" value="" class="frm_input" size="35">
								</td>
							</tr>
						</table>
					</td>
				...
				<tr id="receipt_type2" style="display:none;">
					<th scope="row">영수증 메모</th>
					<td colspan="3"><textarea name="od_receipt_memo" id="od_receipt_memo" placeholder="계산서, 영수증의 사업자번호 등 입력해주세요"></textarea></td>
				</tr>
                ...
                <tr>
                    ...
                    <td colspan="3">
                        <input type="radio" id="od_delivery_gallery_type_1" name="od_delivery_gallery_type"  value="배송후" checked> <label for="od_delivery_gallery_type_1">배송후사진</label>
                        <input type="radio" id="od_delivery_gallery_type_2" name="od_delivery_gallery_type"  value="배송전"> <label for="od_delivery_gallery_type_2">배송전사진</label>
                        <input type="radio" id="od_delivery_gallery_type_3" name="od_delivery_gallery_type"  value="현장"> <label for="od_delivery_gallery_type_3">현장사진</label>
                    </td>
                </tr>
...
// wetoz : mainpay
if ($default['de_pg_service'] == 'mainpay') { // wetoz : mainpay
    require_once(G5_SHOP_PATH.'/mainpay/orderform.append.php');
}
?>
<script>
var zipcode = "";
...
function forderform_check(f)
{
    ...
    if (typeof(f.od_hope_date) != "undefined" && !f.od_hope_now.checked)
    {
    ...
    if( jQuery(f).triggerHandler("form_sumbit_order_"+form_order_method) !== false ) {
        ...
        <?php if ($default['de_pg_service'] == 'mainpay') { // wetoz : mainpay ?>
        var paytype = '';
        switch(settle_method)
        {
            case "계좌이체":
...
$(function(){
    $("#od_hope_date").datepicker({ changeMonth: true, changeYear: true, dateFormat: "yy-mm-dd", showButtonPanel: true, yearRange: "c-99:c+99", minDate: "+<?php echo (int)$default['de_hope_date_after']; ?>d;"});
});
<?php } ?>
</script>