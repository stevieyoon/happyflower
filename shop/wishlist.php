...
<div id="sod_ws">
    ...
    <div class="list_02">
        ...
        $sql .= " where a.mb_id = '{$member['mb_id']}' and a.site_id = '$site_id' and b.site_id = '$site_id' order by a.wi_id desc ";
        $result = sql_query($sql);
        for ($i=0; $row = sql_fetch_array($result); $i++) {

            $out_cd = '';
            $sql = " select count(*) as cnt from {$g5['g5_shop_item_option_table']} where it_id = '{$row['it_id']}' and io_type = '0' and site_id = '$site_id' ";
            $tmp = sql_fetch($sql);
            if($tmp['cnt'])
                $out_cd = 'no';
