...
<div id="coupon" class="new_win">
    ...
    <div class="new_win_con list_01">
        ...
        for($i=0; $row=sql_fetch_array($result); $i++) {
            ...
            if($row['cp_method'] == 1) {
                $sql = " select ca_name from {$g5['g5_shop_category_table']} where ca_id = '{$row['cp_target']}' and site_id = '$site_id' ";
                $ca = sql_fetch($sql);
                ...
                $sql = " select it_name from {$g5['g5_shop_item_table']} where it_id = '{$row['cp_target']}' and site_id = '$site_id' ";
                $it = sql_fetch($sql);
