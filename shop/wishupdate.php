...
if ($w == "d")
{
    ...
    $sql = " select mb_id from {$g5['g5_shop_wish_table']} where wi_id = '$wi_id' and site_id = '$site_id' ";
    $row = sql_fetch($sql);
...
else
{
    ...
    // 상품정보 체크
    $sql = " select it_id from {$g5['g5_shop_item_table']} where it_id = '$it_id' and site_id = '$site_id' ";
    $row = sql_fetch($sql);
    ...
    $sql = " select wi_id from {$g5['g5_shop_wish_table']}
              where mb_id = '{$member['mb_id']}' and it_id = '$it_id' and site_id = '$site_id' ";
    $row = sql_fetch($sql);
