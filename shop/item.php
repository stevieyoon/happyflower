...
$sql = " select a.*, b.ca_name, b.ca_use from {$g5['g5_shop_item_table']} a, {$g5['g5_shop_category_table']} b where a.it_id = '$it_id' and a.ca_id = b.ca_id and a.site_id = '$site_id' ";
$it = sql_fetch($sql);
if (!$it['it_id'])
    alert('자료가 없습니다.');
...
$sql = " select ca_skin_dir, ca_include_head, ca_include_tail, ca_cert_use, ca_adult_use from {$g5['g5_shop_category_table']} where ca_id = '{$it['ca_id']}' and site_id = '$site_id' ";
$ca = sql_fetch($sql);
...
if (get_cookie('ck_it_id') != $it_id) {
    sql_query(" update {$g5['g5_shop_item_table']} set it_hit = it_hit + 1 where it_id = '$it_id' and site_id = '$site_id' "); // 1증가
...
// 이전 상품보기
$sql = " select it_id, it_name from {$g5['g5_shop_item_table']} where it_id > '$it_id' and SUBSTRING(ca_id,1,4) = '".substr($it['ca_id'],0,4)."' and it_use = '1' and site_id = '$site_id' order by it_id asc limit 1 ";
$row = sql_fetch($sql);
if ($row['it_id']) {
...
// 다음 상품보기
$sql = " select it_id, it_name from {$g5['g5_shop_item_table']} where it_id < '$it_id' and SUBSTRING(ca_id,1,4) = '".substr($it['ca_id'],0,4)."' and it_use = '1' and site_id = '$site_id' order by it_id desc limit 1 ";
$row = sql_fetch($sql);
if ($row['it_id']) {
...
$sql = " select count(*) as cnt from `{$g5['g5_shop_item_use_table']}` where it_id = '{$it_id}' and is_confirm = '1' and site_id = '$site_id' ";
$row = sql_fetch($sql);
$item_use_count = $row['cnt'];

// 상품문의의 개수를 얻음
$sql = " select count(*) as cnt from `{$g5['g5_shop_item_qa_table']}` where it_id = '{$it_id}' and site_id = '$site_id' ";
$row = sql_fetch($sql);
$item_qa_count = $row['cnt'];

// 관련상품의 개수를 얻음
if($default['de_rel_list_use']) {
    $sql = " select count(*) as cnt from {$g5['g5_shop_item_relation_table']} a left join {$g5['g5_shop_item_table']} b on (a.it_id2=b.it_id) where a.it_id = '{$it['it_id']}' and  b.it_use='1' and a.site_id = '$site_id' ";
    $row = sql_fetch($sql);
    $item_relation_count = $row['cnt'];
}

// 배송사진의 개수를 얻음
$sql = " select count(*) as cnt from `{$g5['delivery_gallery']}` where it_id = '{$it_id}' and site_id = '$site_id' ";
$row = sql_fetch($sql);
...
function pg_anchor($anc_id) {
    global $default;
    global $item_use_count, $item_qa_count, $item_relation_count, $delivery_count;
?>
    <ul class="sanchor">
        <li><a href="#sit_inf" <?php if ($anc_id == 'inf') echo 'class="sanchor_on"'; ?>>상품정보</a></li>
        <li><a href="#sit_gallery" <?php if ($anc_id == 'delivery') echo 'class="sanchor_on"'; ?>>배송사진 <span class="item_use_count"><?php echo $delivery_count; ?></span></a></li>
