...
else if ($w == "u")
{
    if (!$is_admin)
    {
        $sql = " select count(*) as cnt from {$g5['g5_shop_item_qa_table']} where mb_id = '{$member['mb_id']}' and iq_id = '$iq_id' and site_id = '$site_id' ";
        $row = sql_fetch($sql);
        if (!$row['cnt'])
    ...
    $sql = " update {$g5['g5_shop_item_qa_table']}
                set iq_secret = '$iq_secret',
                    ...
                    iq_subject = '$iq_subject',
                    iq_question = '$iq_question'
              where iq_id = '$iq_id' and site_id = '$site_id' ";
    sql_query($sql);
...
else if ($w == "d")
{
    if (!$is_admin)
    {
        $sql = " select iq_answer from {$g5['g5_shop_item_qa_table']} where mb_id = '{$member['mb_id']}' and iq_id = '$iq_id' and site_id = '$site_id' ";
        $row = sql_fetch($sql);
        if (!$row)
    ...
    // 에디터로 첨부된 이미지 삭제
    $sql = " select iq_question, iq_answer from {$g5['g5_shop_item_qa_table']} where iq_id = '$iq_id' and md5(concat(iq_id,iq_time,iq_ip)) = '{$hash}' and site_id = '$site_id' ";
    $row = sql_fetch($sql);
    ...
    $sql = " delete from {$g5['g5_shop_item_qa_table']} where iq_id = '$iq_id' and md5(concat(iq_id,iq_time,iq_ip)) = '{$hash}' and site_id = '$site_id' ";
    sql_query($sql);
