...
$sql = " select it_id,
          ...
          where od_id = '$tmp_cart_id'
            and ct_select = '1' and site_id = '$site_id' ";
...
// 리본 테이블 저장(gnuwiz)
if($_POST['ribbon']) {
    for ($k=0; $k<count($_POST['ribbon']); $k++) {

        $m += 1;
        $rb_left_content = 	$_POST['rb_left_content'][$m];
        $rb_right_content = $_POST['rb_right_content'][$m];
        $it_name =  $_POST['it_name'][$m];
        $it_id = $_POST['it_id'][$m];
        $card_msg = $_POST['card_msg'][$m];
        $ribbon_type = $_POST['ribbon_type'][$m];
...
// 주문 정보 임시 데이터 삭제
if($od_pg == 'inicis') {
    $sql = " delete from {$g5['g5_shop_order_data_table']} where od_id = '$od_id' and dt_pg = '$od_pg' and site_id = '$site_id' ";
    sql_query($sql);
}
...
if($is_member) {
    ...
    if($ad_default) {
        $sql = " update {$g5['g5_shop_order_address_table']}
                    set ad_default = '0'
                    where mb_id = '{$member['mb_id']}' and site_id = '$site_id' ";
        sql_query($sql);
    }
    ...
    if($row['ad_id']){
        $sql = " update {$g5['g5_shop_order_address_table']}
                    ...
                    where mb_id = '{$member['mb_id']}'
                      and ad_id = '{$row['ad_id']}' and site_id = '$site_id' ";
    } else {
...
// CTI api 전송(gnuwiz)
//include_once(G5_SHOP_PATH.'/cti_post.php');
