...
if($act == "buy")
{
    ...
    // 선택필드 초기화
    $sql = " update {$g5['g5_shop_cart_table']} set ct_select = '0' where od_id = '$tmp_cart_id' and site_id = '$site_id' ";
    sql_query($sql);
        ...
        if($ct_chk) {
            ...
            $sql = " select ct_qty, it_name, ct_option, io_id, io_type
                        ...
                        where od_id = '$tmp_cart_id'
                          and it_id = '$it_id' and site_id = '$site_id' ";
...
else if ($act == "alldelete") // 모두 삭제이면
{
    $sql = " delete from {$g5['g5_shop_cart_table']}
              where od_id = '$tmp_cart_id' and site_id = '$site_id' ";
    sql_query($sql);
}
else if ($act == "seldelete") // 선택삭제
{
    ...
    for($i=0; $i<$fldcnt; $i++) {
        ...
        if($ct_chk) {
            $it_id = $_POST['it_id'][$i];
            $sql = " delete from {$g5['g5_shop_cart_table']} where it_id = '$it_id' and od_id = '$tmp_cart_id' and site_id = '$site_id' ";
            sql_query($sql);
        }
    }
}
else // 장바구니에 담기
{
    ...
    for($i=0; $i<$count; $i++) {
        ...
        // 상품정보
        $sql = " select * from {$g5['g5_shop_item_table']} where it_id = '$it_id' and site_id = '$site_id' ";
        $it = sql_fetch($sql);
        if(!$it['it_id'])
        ...
        if($i == 0 && $sw_direct)
            sql_query(" delete from {$g5['g5_shop_cart_table']} where od_id = '$tmp_cart_id' and ct_direct = 1 and site_id = '$site_id' ", false);
        ...
        // 옵션정보를 얻어서 배열에 저장
        $opt_list = array();
        $sql = " select * from {$g5['g5_shop_item_option_table']} where it_id = '$it_id' and io_use = 1 and site_id = '$site_id' order by io_no asc ";
        $result = sql_query($sql);
        $lst_count = 0;
        for($k=0; $row=sql_fetch_array($result); $k++) {
        ...
        if($act == 'optionmod')
            sql_query(" delete from {$g5['g5_shop_cart_table']} where od_id = '$tmp_cart_id' and it_id = '$it_id' and site_id = '$site_id' ");
        ...
        $sql = " INSERT INTO {$g5['g5_shop_cart_table']}
                        ( od_id, mb_id, it_id, it_name, it_sc_type, it_sc_method, it_sc_price, it_sc_minimum, it_sc_qty, ct_status, ct_price, ct_point, ct_point_use, ct_stock_use, ct_option, ct_qty, ct_notax, io_id, io_type, io_price, ct_time, ct_ip, ct_send_cost, ct_direct, ct_select, ct_select_time, site_id )
                    VALUES ";
        ...
        for($k=0; $k<$opt_count; $k++) {
            ...
            if($row2['ct_id']) {
                ...
                $sql3 = " update {$g5['g5_shop_cart_table']}
                            ...
                            where ct_id = '{$row2['ct_id']}' and site_id = '$site_id' ";
                sql_query($sql3);
                continue;
            }
            ...
            $sql .= $comma."( '$tmp_cart_id', '{$member['mb_id']}', '{$it['it_id']}', '".addslashes($it['it_name'])."', '{$it['it_sc_type']}', '{$it['it_sc_method']}', '{$it['it_sc_price']}', '{$it['it_sc_minimum']}', '{$it['it_sc_qty']}', '쇼핑', '{$it['it_price']}', '$point', '0', '0', '$io_value', '$ct_qty', '{$it['it_notax']}', '$io_id', '$io_type', '$io_price', '".G5_TIME_YMDHIS."', '$remote_addr', '$ct_send_cost', '$sw_direct', '$ct_select', '$ct_select_time', '$site_id' )";
            $comma = ' , ';
            $ct_count++;
        }
