...
$sql_common = " from {$g5['g5_shop_order_table']} where site_id = '$site_id' $sql_search ";
...
<div class="tbl_head01 tbl_wrap">
    ...
    <tr>
        ...
        <th scope="col" rowspan="3">카드영수증</th>
    ...
    for ($i=0; $row=sql_fetch_array($result); $i++)
    {
        ...
        // 영수증 출력(gnuwiz)
        if ($row['od_misu'] == 0 && $row['od_receipt_price'] && ($row['od_settle_case'] == '무통장' || $row['od_settle_case'] == '계좌이체' || $row['od_settle_case'] == '가상계좌'))
        {
            if ($row['od_cash'])
            {
                if($row['od_pg'] == 'lg') {
                    ...
                    $cash_receipt_script = 'javascript:showCashReceipts(\''.$LGD_MID.'\',\''.$row['od_id'].'\',\''.$row['od_casseqno'].'\',\''.$trade_type.'\',\''.$CST_PLATFORM.'\');';
                } else if($row['od_pg'] == 'inicis') {
                    $cash = unserialize($row['od_cash_info']);
                    $cash_receipt_script = 'window.open(\'https://iniweb.inicis.com/DefaultWebApp/mall/cr/cm/Cash_mCmReceipt.jsp?noTid='.$cash['TID'].'&clpaymethod=22\',\'showreceipt\',\'width=380,height=540,scrollbars=no,resizable=no\');';
                }else if($row['od_pg'] == 'mainpay') {
                    $cash = unserialize($row['od_cash_info']);
                    $cash_receipt_script = 'window.open(\'https://dev-biz.mainpay.co.kr/card/cashReceiptPopUp.do??ref_no='.$row["od_tno"].'& section=01\',\'showreceipt\',\'width=380,height=540,scrollbars=no,resizable=no\');';
                } else {
                    require_once G5_SHOP_PATH.'/settle_kcp.inc.php';

                    $cash = unserialize($row['od_cash_info']);
                    $cash_receipt_script = 'window.open(\''.G5_CASH_RECEIPT_URL.$shop_default['de_kcp_mid'].'&orderid='.$row['od_id'].'&bill_yn=Y&authno='.$cash['receipt_no'].'\', \'taxsave_receipt\', \'width=360,height=647,scrollbars=0,menus=0\');';
                }

                $od_settle_receipt = '<a href="javascript:;" onclick="'.$cash_receipt_script.'" class="btn_frmline_yellow">현금영수증 확인하기</a>';

            }
            else
            {
                $taxsavea = G5_SHOP_URL.'/taxsave.php?od_id='.$row['od_id'];
                $od_settle_receipt = "<a href=\"javascript:;\" onclick=\"window.open('".$taxsavea."', 'taxsave', 'width=550,height=600,scrollbars=1,menus=0');\" class=\"btn_frmline_yellow\">현금영수증을 발급하시려면 클릭하십시오.</a>";
            }
            ...
            if($row['od_settle_case'] == '휴대폰')
            {
                if($row['od_pg'] == 'lg') {
                    require_once G5_SHOP_PATH.'/settle_lg.inc.php';
                    $LGD_TID      = $row['od_tno'];
                    $LGD_MERTKEY  = $shop_default['cf_lg_mert_key'];
                    $LGD_HASHDATA = md5($LGD_MID.$LGD_TID.$LGD_MERTKEY);

                    $hp_receipt_script = 'showReceiptByTID(\''.$LGD_MID.'\', \''.$LGD_TID.'\', \''.$LGD_HASHDATA.'\');';
                }else if($row['od_pg'] == 'mainpay') {
                    $card_receipt_script = 'window.open(\'https://biz.mainpay.co.kr/card/cardReceipt_popup.do?ref_no='.$row["od_tno"].'&tran_date='.$od_receipt_time.'\',\'receipt\',\'toolbar=no,location=no,directories=no,status=yes,menubar=no,scrollbars=yes,resizable=yes,width=730,height=700\');';
                } else if($row['od_pg'] == 'inicis') {
                    $hp_receipt_script = 'window.open(\'https://iniweb.inicis.com/DefaultWebApp/mall/cr/cm/mCmReceipt_head.jsp?noTid='.$row['od_tno'].'&noMethod=1\',\'receipt\',\'toolbar=no,location=no,directories=no,status=yes,menubar=no,scrollbars=yes,resizable=yes,width=430,height=700\');';
                } else {
                    $hp_receipt_script = 'window.open(\''.G5_BILL_RECEIPT_URL.'mcash_bill&tno='.$row['od_tno'].'&order_no='.$row['od_id'].'&trade_mony='.$row['od_receipt_price'].'\', \'winreceipt\', \'toolbar=no,location=no,directories=no,status=yes,menubar=no,scrollbars=yes,resizable=yes,width=500,height=690,scrollbars=yes,resizable=yes\');';
                }

                $od_settle_receipt = '<a href="javascript:;" onclick="'.$hp_receipt_script.'">휴대폰 영수증 출력</a>';
            ...
            if($row['od_settle_case'] == '신용카드')
            {
                if($row['od_pg'] == 'lg') {
                    ...
                    $card_receipt_script = 'window.open(\'https://iniweb.inicis.com/DefaultWebApp/mall/cr/cm/mCmReceipt_head.jsp?noTid='.$row['od_tno'].'&noMethod=1\',\'receipt\',\'toolbar=no,location=no,directories=no,status=yes,menubar=no,scrollbars=yes,resizable=yes,width=430,height=700\');';
                } else if($row['od_pg'] == 'mainpay') {
                    $card_receipt_script = 'window.open(\'https://biz.mainpay.co.kr/card/cardReceipt_popup.do?ref_no='.$row["od_tno"].'&tran_date='.$od_receipt_time.'\',\'receipt\',\'toolbar=no,location=no,directories=no,status=yes,menubar=no,scrollbars=yes,resizable=yes,width=730,height=700\');';
                } else {
                    $card_receipt_script = 'window.open(\''.G5_BILL_RECEIPT_URL.'card_bill&tno='.$row['od_tno'].'&order_no='.$row['od_id'].'&trade_mony='.$row['od_receipt_price'].'&order_nm='.$row['od_name'].'&app_mony='.$row['od_receipt_price'].'\', \'winreceipt\', \'toolbar=no,location=no,directories=no,status=yes,menubar=no,scrollbars=yes,resizable=yes,width=470,height=815,scrollbars=yes,resizable=yes\');';
                }
                $od_settle_receipt = '<a href="javascript:;" onclick="'.$card_receipt_script.'" class="btn_frmline_blue">카드영수증 출력</a>';
            }

            if($row['od_settle_case'] == 'KAKAOPAY')
            {
                $card_receipt_script = 'window.open(\'https://mms.cnspay.co.kr/trans/retrieveIssueLoader.do?TID='.$row['od_tno'].'&type=0\', \'popupIssue\', \'toolbar=no,location=no,directories=no,status=yes,menubar=no,scrollbars=yes,resizable=yes,width=420,height=540\');';
                $od_settle_receipt = '<a href="javascript:;" onclick="'.$card_receipt_script.'">카카오 영수증 출력</a>';
            }
        }
    ?>
    <tr class="orderlist<?php echo ' '.$bg; ?>">
        ...
        <td rowspan="3" class="td_name"><?php echo $od_settle_receipt; ?></td>
