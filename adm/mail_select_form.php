...
// 전체회원수
$sql = " select COUNT(*) as cnt from {$g5['member_table']} where site_id = '{$site_id}' ";
$row = sql_fetch($sql);
$tot_cnt = $row['cnt'];

// 탈퇴대기회원수
$sql = " select COUNT(*) as cnt from {$g5['member_table']} where mb_leave_date <> '' and site_id = '{$site_id}' ";
$row = sql_fetch($sql);
$finish_cnt = $row['cnt'];
