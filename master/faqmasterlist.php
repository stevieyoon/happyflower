...
$sql_common = " from {$g5['faq_master_table']} where site_id = '$site_id' ";
...
<div class="tbl_head01 tbl_wrap">
    ...
    <?php for ($i=0; $row=sql_fetch_array($result); $i++) {
        $sql1 = " select COUNT(*) as cnt from {$g5['faq_table']} where fm_id = '{$row['fm_id']}' and site_id = '$site_id' ";
        $row1 = sql_fetch($sql1);
        $cnt = $row1['cnt'];
        $bg = 'bg'.($i%2);
    ?>
    <tr class="<?php echo $bg; ?>">
		...
		<td class="td_num"><?php echo ($rows * ($page-1)) + $i+1; ?></td>
