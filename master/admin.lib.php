...
if($config['cf_admin'] != "master") alert("접근 하실 수 없습니다","/");
...
function get_member_id_select($name, $level, $selected="", $event="")
{
    ...
    $sql = " select mb_id from {$g5['member_table']} where mb_level >= '{$level}' and site_id = '{$config['site_id']}' ";
...
function icon($act, $link='', $target='_parent')
{
    ...
    $icon = '<img src="'.G5_MASTER_PATH.'/img/icon_'.$img[$act].'.gif" title="'.$act.'">';
    if ($link)
...
function get_master_token()
{
    ...
    set_session('ss_master_token', $token);

    return $token;
}

// 관리자가 자동등록방지를 사용해야 할 경우
function get_master_captcha_by($type='get'){

    $captcha_name = 'ss_master_use_captcha';
...
function check_master_token()
{
    $token = get_session('ss_master_token');
...
function admin_referer_check($return=false)
{
    ...
    if( $p['path'] && ! preg_match( '/\/'.preg_quote(G5_MASTER_DIR).'\//i', $p['path'] ) ){
        $msg = '올바른 방법으로 이용해 주십시오';
    }
...
if (!$member['mb_id'])
{
    alert('로그인 하십시오.', G5_BBS_URL.'/login.php?url=' . urlencode(G5_MASTER_URL));
}
...
while ($entry = $tmp->read()) {
    ...
    $amenu[$m[1]] = $entry;
    $menu_files[] = G5_MASTER_PATH.'/'.$entry;
}
@asort($menu_files);
