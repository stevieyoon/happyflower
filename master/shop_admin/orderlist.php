...
<div class="tbl_head01 tbl_wrap">
    ...
    <tr>
        ...
        <th scope="col" rowspan="3">사이트이름</th>
    ...
    for ($i=0; $row=sql_fetch_array($result); $i++)
    {
        ...
        ############################ 그누위즈 추가 시작
        $od_settle_receipt = '';
        if ($misu_price == 0 && $row['od_receipt_price'])
        {
            // 주문상태 값
            switch ($row['od_settle_case']) {
                case '무통장':
                    $taxsavea = G5_SHOP_URL.'/taxsave.php?od_id='.$row['od_id'];
                    $od_settle_receipt = "<a href=\"javascript:;\" onclick=\"window.open('".$taxsavea."', 'taxsave', 'toolbar=no,location=no,directories=no,status=yes,menubar=no,scrollbars=yes,resizable=yes,width=750,height=500,scrollbars=1,menus=0');\" class=\"btn_frmline\">현금영수증</a>";
                    break;
                case '신용카드':
                    $ct_status = '배송';
                    break;
    ...
    <tr class="orderlist<?php echo ' '.$bg; ?>">
        ...
        <td rowspan="3" class="td_name">
            <?php
            $site_info = get_config($row['site_id']);
            echo $site_info['cf_title'];
            ?>
        </td>
