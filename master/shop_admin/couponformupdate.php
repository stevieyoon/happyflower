...
if($_POST['cp_method'] == 0) {
    $sql = " select count(*) as cnt from {$g5['g5_shop_item_table']} where it_id = '$cp_target' and it_nocoupon = '0' and site_id = '$site_id' ";
    $row = sql_fetch($sql);
    if(!$row['cnt'])
        alert('입력하신 상품코드는 존재하지 않는 코드이거나 쿠폰적용안함으로 설정된 상품입니다.');
} else if($_POST['cp_method'] == 1) {
    $sql = " select count(*) as cnt from {$g5['g5_shop_category_table']} where ca_id = '$cp_target' and ca_nocoupon = '0' and site_id = '$site_id' ";
    $row = sql_fetch($sql);
    if(!$row['cnt'])
...
if($w == '') {
    if($_POST['chk_all_mb']) {
        ...
        $sql = " select mb_id from {$g5['member_table']} where mb_id = '{$_POST['mb_id']}' and mb_leave_date = '' and mb_intercept_date = '' and site_id = '$site_id' ";
        $row = sql_fetch($sql);
        if(!$row['mb_id'])
    ...
    do {
        ...
        $sql3 = " select count(*) as cnt from {$g5['g5_shop_coupon_table']} where cp_id = '$cp_id' and site_id = '$site_id' ";
        $row3 = sql_fetch($sql3);
    ...
    $sql = " INSERT INTO {$g5['g5_shop_coupon_table']}
                ( cp_id, cp_subject, cp_method, cp_target, mb_id, cp_start, cp_end, cp_type, cp_price, cp_trunc, cp_minimum, cp_maximum, cp_datetime, site_id )
            VALUES
                ( '$cp_id', '$cp_subject', '$cp_method', '$cp_target', '$mb_id', '$cp_start', '$cp_end', '$cp_type', '$cp_price', '$cp_trunc', '$cp_minimum', '$cp_maximum', '".G5_TIME_YMDHIS."', '$site_id' ) ";

    sql_query($sql);
} else if($w == 'u') {
    $sql = " select * from {$g5['g5_shop_coupon_table']} where cp_id = '$cp_id' and site_id = '$site_id' ";
    $cp = sql_fetch($sql);
    ...
    $sql = " update {$g5['g5_shop_coupon_table']}
                ...
                where cp_id = '$cp_id' and site_id = '$site_id' ";
    sql_query($sql);
}
